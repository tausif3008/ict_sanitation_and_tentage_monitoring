...
import {
  ...
  getVendorTypeRegData,
} from "../VendorwiseReports/vendorslice";
import ExportToExcel from "../ExportToExcel";

const VendorTypeReport = () => {
  ...
  const { VendorCatTypeDrop, VendorTypeRegReport_data, VendorReport_Loading } =
    VendorSelectors(); // vendor dropdown & Vendor-type Wise Report data
  ...
  // file name
  const getReportName = () => {
    let name = "Vendor & Type";
    if (vendorName) {
      name = `${vendorName} Vendor & Type`;
    }
    if (catTypeName) {
      name += `- ${catTypeName}`;
    }
  ...
  const onFinishForm = (values) => {
    ...
    dispatch(getVendorTypeRegData(formData)); // Fetch the data
  };
  ...
  const getData = async () => {
    ...
    dispatch(getVendorTypeRegData(finalData)); // Fetch the data
    handleSelect("1");
    form.setFieldsValue({
      asset_main_type_id: "1",
      asset_type_ids: fiveTypes?.[0]?.value,
  ...
  const memoizedColumns = useMemo(() => {
    if (!VendorTypeRegReport_data?.data?.listings) return [];
    const columnNames =
      VendorTypeRegReport_data?.data?.listings?.[0]?.asset_types?.map(
        (item, index) => {
          return {
  ...
  }, [VendorTypeRegReport_data]); // Only recompute when VendorTypeRegReport_data changes

  useEffect(() => {
    if (VendorTypeRegReport_data) {
      let sectorCount = {};
      const tableData = VendorTypeRegReport_data?.data?.listings?.map(
        (item) => {
          const sectorData = {
            id: item?.vendor_id,
          ...
          sectorData[`vendor_total`] = count;
          sectorCount[item?.vendor_name] = count;
          return sectorData;
        }
      );

      let myCount = {};
      VendorTypeRegReport_data?.data?.listings?.forEach((item) => {
      ...
      setVendorData((prevDetails) => ({
        ...prevDetails,
        list: tableData || [],
        pageLength: VendorTypeRegReport_data?.data?.paging?.[0]?.length || 0,
        currentPage:
          VendorTypeRegReport_data?.data?.paging?.[0]?.currentpage || 1,
        totalRecords:
          VendorTypeRegReport_data?.data?.paging?.[0]?.totalrecords || 0,
      }));
    }
  ...
  const pdfData = useMemo(() => {
    return vendorData?.list?.map((item, index) => {
      const row = [index + 1, item?.vendor_name]; // Start with the row number (index + 1)
      Object.keys(item).forEach((key) => {
        if (key.startsWith("dataIndex")) {
          row.push(item[key]); // Add the value to the row
        } else if (key.startsWith("vendor_total")) {
          row.push(item["vendor_total"]); // Add the value to the row
        }
      });
  ...
  return (
    <div>
      <CommonDivider label={"Vendor & Type Wise Registration Report"} />
      <div className="flex justify-end gap-2 font-semibold">
        <ExportToPDF
          titleName={`Vendor & Type Wise Registration Report (${fileDateName})`}
          pdfName={
            fileName ? fileName : "Vendor & Type Wise Registration Report"
          }
...
export default VendorTypeReport;
